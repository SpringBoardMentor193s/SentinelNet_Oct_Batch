...
def get_classifier_options():
    """Return available ML classifiers."""
    return {
        "Random Forest": RandomForestClassifier(n_estimators=200, max_depth=None, n_jobs=-1, random_state=42),
...
def main():
    ...
    with tab1:
        ...
        with st.expander("🔍 Dataset Preview", expanded=True):
            st.dataframe(dataset.head(15), use_container_width=True, height=400)
    ...
    with tab2:
        ...
        if st.button("▶ Start Training Process", type="primary", use_container_width=True):
            ...
            try:
                ...
                # Step 4: Training
                status.markdown("**🔄 Step 4/4:** Training classifier...")
                ...
                # Test Set Class Distribution (ONLY ONE - after metrics)
                ...
                with col2:
                    cm_chart = create_confusion_heatmap(cm, encoder.classes_)
                ...
                class_report_str = classification_report(y_test, y_pred, target_names=encoder.classes_)
